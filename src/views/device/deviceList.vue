<template>
  <div class="content">
    ...
    <device-detail ref="detail" @ok="handleOk"></device-detail>
  </div>
</template>
...
import { deviceTypeData, delDeviceTypeByIds } from '@/api/device'
export default {
  name: 'DeviceList',
  props: {
    isParent: {
  ...
  data () {
    return {
      ...
      loadData: parameter => {
        return deviceTypeData(this.queryParam).then(res => {
  ...
  methods: {
    ...
    handleDelete (record) {
      ...
      this.$confirm({
        ...
        onOk () {
          console.log(ids)
          return delDeviceTypeByIds(ids).then(res => {
            if (res.success === true) {
