<template>
  <div class="content">
    ...
    <resource-deatil ref="detail" @ok="handleOk"></resource-deatil>
  </div>
</template>

<script>
import { STable } from '@/components'
import resourceDeatil from './resourceDeatil'
import { resourcLabData, delResourceLabByIds } from '@/api/resource'
export default {
  name: 'ResourceLab',
  props: {
    isParent: {
  ...
  data () {
    return {
      ...
      loadData: parameter => {
        return resourcLabData(this.queryParam).then(res => {
  ...
  methods: {
    ...
    handleDelete (record) {
      ...
      this.$confirm({
        ...
        onOk () {
          console.log(ids)
          return delResourceLabByIds(ids).then(res => {
            if (res.success === true) {
